$(document).ready(function () {
    ...
    changetxt_chkbox("#chkuseemail", "Ha seleccionado: Ingresar su Correo registrado para la Consulta", "Ha seleccionado: Usar su número de documento registrado para la Consulta");
    ...
    $editor.submit(function (e) {
        ...
        if (is_curso_libre) { //SI ES UN CURSO DE LIBRE ACCESO
            //VALIDANDO TIPO DE PARTICIPANTE
            if ($("#chkprivado").prop("checked")) { //SE ACTIVO CHKPRIVADO
                var valpriv_inst = $("#ext_institucion").val() == null ? "" : $("#ext_institucion").val();
                var valpriv_prof = $("#ext_profesion").val() == null ? "" : $("#ext_profesion").val();
                if (valpriv_inst != "" && valpriv_prof != "") {
                    v_tipopart = "privado";
                }
            }
        }
    ...
    function badge_detalle(texto) {
        var hmtl_badge = "";
        if (texto == "NINGUNA") {
            hmtl_badge = "<span class='badge rounded-pill bg-danger' >" + texto + "</span>";
        } else {
            hmtl_badge = texto;
        }
        return hmtl_badge;
    ...
    function changetxt_chkbox(chknom, txton, txtoff) {
        $(chknom).on("change", function (e) {
            chk = $(this);
            estado = chk.prop("checked");
            if (estado) {
                chk.next().html("<font class='text-success'>" + txton + "</font>");
            } else {
                chk.next().html("<font class='text-primary'>" + txtoff + "</font>");
            }
        });
    }
            ...
            success: function (data) {
                ...
                if (data == false) {
                    ...
                    $.each(data, function (i, item) {
                        ...
                        tabla += "<td>" + badge_detalle(item.cursodetalle) + "</td>";
                        tabla += "</tr>";
                    });
                    Swal.close();
                }
